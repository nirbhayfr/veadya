...
const AdminPanel = () => {
	...
	const handleGenerateDetails = async () => {
		if (!canGenerateDetails) {
			setErrorMsg(
				"Upload an image and enter the product name and short description first.",
			);
			return;
		}
		...
		try {
			...
			setSuccessMsg(
				"AI details generated. Review or edit them before saving.",
			);
		} catch (err) {
			setErrorMsg(
				err.message || "Failed to generate product details.",
			);
		} finally {
	...
	const handleSaveProduct = async (e) => {
		...
		try {
			...
			const payload = {
				...
				tags: tagsArray,
				images: productForm.images.map((imgUrl) => ({
					url: imgUrl,
				})),
				bg: productForm.bg,
		...
		>
			...
			<aside className="w-64 bg-[#114232] text-white flex flex-col shadow-xl max-h-screen sticky top-0">
				<div className="p-6 border-b border-white/10 flex items-center justify-between">
					<div>
						<h1 className="text-xl font-serif font-bold text-[#e2d5c3] tracking-wide">
							Veadya Admin
						</h1>
				...
				<header className="bg-white border-b border-gray-100 px-8 py-5 flex justify-between items-center shadow-xs">
					<div>
						...
						<p className="text-xs text-gray-400 mt-1">
							Manage the online botanical apothecary store.
						</p>
					</div>
									...
									>
										<div className="grid grid-cols-2 gap-6">
											<div className="space-y-2">
												<label className="text-xs uppercase tracking-wider text-[#114232] font-semibold block">
													Product Title *
												</label>
												<input
													type="text"
										...
										<div className="space-y-4 pt-4 border-t border-gray-50">
											...
											<div className="grid grid-cols-5 gap-4">
												{productForm.images.map(
													(
														...
														<div
															...
															className="aspect-square bg-gray-50 border border-gray-200 rounded-xl overflow-hidden relative group"
														>
															<img
																src={
																	imgUrl
												...
												{productForm.images
													...
													5 && (
													<label
														className={`aspect-square border-2 border-dashed border-gray-300 hover:border-[#114232] rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all hover:bg-gray-50 ${uploadingImage ? "opacity-50 cursor-wait" : ""}`}
													>
														<input
															type="file"
															accept="image/*"
														...
														/>
														{uploadingImage ? (
															<span className="text-[10px] text-gray-400 text-center font-medium animate-pulse">
																Uploading...
															</span>
														) : (
															<>
										...
										<div className="grid grid-cols-4 gap-4 pt-4 border-t border-gray-50">
											<div className="space-y-1.5">
												<label className="text-[10px] uppercase tracking-wider text-gray-400 block">
													Theme BG
												</label>
												<input
													type="color"
														...
														(p) => (
															...
															>
																...
																<td className="py-4 px-6 font-semibold">
																	...
																	{p.originalPrice && (
																		<span className="text-xs font-normal text-gray-400 line-through ml-1.5">
																			₹
																			{
																				p.originalPrice
																			}
																...
																<td className="py-4 px-6 text-center">
																	<span
																		className={`font-semibold font-mono ${p.stock === 0 ? "text-red-500" : p.stock <= 5 ? "text-amber-500" : "text-gray-600"}`}
																	>
																		{
																			p.stock
																		}
												...
												(order) => (
													...
													>
														...
														<td className="py-4 px-6">
															...
															<span
																className={`block w-fit mt-2 text-[10px] uppercase font-bold tracking-wider px-2.5 py-1.5 rounded-lg border ${getStatusBadgeClass(order.paymentStatus)}`}
															>
																{
																	order.paymentStatus
																}
