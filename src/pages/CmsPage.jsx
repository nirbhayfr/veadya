...
export const CmsPage = () => {
  ...
  return <article className="section-container py-20 max-w-4xl mx-auto">
    {page.featuredImage && <img src={page.featuredImage} alt={page.title} className="w-full max-h-[480px] object-cover rounded-3xl mb-10" />}
...
export const JournalPost = () => {
  ...
  return <article className="section-container py-20 max-w-4xl mx-auto">{post.featuredImage && <img src={post.featuredImage} alt={post.title} className="w-full max-h-[520px] object-cover rounded-3xl mb-10" />}<p className="text-xs uppercase tracking-[0.2em] text-[#114232]">The Journal</p><h1 className="text-5xl font-serif mt-3">{post.title}</h1><p className="text-lg text-gray-500 mt-5">{post.excerpt}</p><div className="mt-10 text-gray-700 leading-8 whitespace-pre-wrap">{post.content}</div></article>;
};
