...
const FullVeadyaEdit = () => {
	...
	const displayProducts = products.slice(3);
			...
			<div className="section-container relative z-10">
				...
				<div className="text-center mb-12">
					<p className="text-[#9abcb9] text-[10px] uppercase tracking-[0.25em] font-medium mb-4 flex items-center justify-center gap-2">
						...
						{content.eyebrow || "Explore the Veadya Collection"}
					</p>
					<h2 className="font-serif text-[42px] md:text-[56px] leading-[1.1] mb-4 font-normal text-white">
						{content.title || "Our Everyday Wellness Collection"}
					</h2>
					<p className="text-[#9abcb9] text-[15px] font-light tracking-wide">
						{content.description ||
							"A thoughtfully selected range of Ayurvedic products created for different everyday wellness needs."}
					</p>
				</div>
					...
					>
						{displayProducts.map((product) => (
							<SwiperSlide
								...
								className="border-r border-white/10 h-auto flex"
							>
								<ProductCard
									product={product}
									isDark={true}
				...
				<div className="text-center">
					<Link
						...
						className="inline-flex items-center gap-3 bg-[#f3eed5] text-[var(--bg-deep)] px-8 py-3.5 rounded-sm font-semibold text-[10px] uppercase tracking-[0.2em] hover:bg-white transition-all shadow-lg"
					>
						→ {content.buttonText || "VIEW THE COMPLETE COLLECTION"}
					</Link>
				</div>
			</div>
		</section>
